        ...
        for (let transaction of block.transactions) {
            ...
            for (let output of transaction.outputs) {
                if (output.scriptPubKey.addresses)
                    addresses[output.scriptPubKey.addresses[0].replace(/\s/g, '')] = -1;
            }
        }
    }
    return addresses;
...
function saveTransactionOutputs(ground, outputs, addresses) {
    ...
    const transactionClauses = outputs.filter(association => association.output.scriptPubKey.addresses).map(association => {
        return createTxOutSQL(association, addresses[association.output.scriptPubKey.addresses[0]]);
    });
    const sql = header + transactionClauses.join(',\n') + ' ON CONFLICT DO NOTHING;';
    return ground.querySingle(sql);
}
function createTxOutSQL(association, addressId) {

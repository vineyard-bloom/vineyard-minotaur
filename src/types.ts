import {
  BlockInfo, ExternalSingleTransaction as ExternalTransaction, SingleTransaction as Transaction,
  TransactionStatus
} from "vineyard-blockchain"
import {TransactionToSave} from "./deposit-monitor-manager";
...
export type TransactionQueryDelegate = (txid: string) => Promise<Transaction | undefined>

export type TransactionSaveDelegate<Identity, AddressIdentity, BlockIdentity> =
  (transaction: Partial<SingleTransaction<Identity, AddressIdentity, BlockIdentity>>) => Promise<void>

export type TransactionStatusDelegate = (transaction: Transaction, status: TransactionStatus) => Promise<Transaction>

export type PendingTransactionDelegate = (maxBlockIndex: number) => Promise<Transaction[]>
